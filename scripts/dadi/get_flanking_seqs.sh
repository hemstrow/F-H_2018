...
snp_ac=~/monarch/github/F-H_2018/data/dadi_inputs/rand_10kgap_snps.txt
anc=~/monarch/Erp_BRA_16005/D_erippus.fa
ref=~/genomes/Dp_genome_v3.fasta
outdir=~/monarch/github/F-H_2018/data/dadi_inputs/
...
samtools faidx $ref $q | awk -v ADD="---" -v PATTERN="^>" 'L { print L; if((L ~ PATTERN) && ($0 ~ PATTERN)) print ADD }; { L=$0 } END { print L }' | grep -v "^>" > ${outdir}ref_flank.txt

echo "Querying ancestral fasta."
samtools faidx $anc $q | awk -v ADD="---" -v PATTERN="^>" 'L { print L; if((L ~ PATTERN) && ($0 ~ PATTERN)) print ADD }; { L=$0 } END { print L }' | grep -v "^>" > ${outdir}anc_flank.txt
echo "Done."
